...
Процедура СЗЗаданиеОбработкаВыбораНаСервере(ДЗПроизв,ДЗНоменклатура,ДЗПериод,ДЗСерия)
	...
	Если ТабЗнач.Количество()>=1 Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |ГДЕ
		               |	ШтрихкодыНоменклатуры.Номенклатура = &Номенклатура
		               |	И ШтрихкодыНоменклатуры.Упаковка.Наименование = &Упаковка";
		
		Запрос.УстановитьПараметр("Номенклатура", Спр.Ссылка);
		Запрос.УстановитьПараметр("Упаковка", Упаковка.Наименование);
...
&НаКлиенте
Процедура ПечатьКазахстан(Команда)
		Ош = ПроверкаПолей();
	Если Ош = Ложь Тогда
		ТаблДокум = Новый  ТабличныйДокумент;
		ТаблДокум = ПечатьНаСервереКазахстан(ТаблДокум);
		ТаблДокум.Показать("Печать документа");
	Иначе
		ДиалогОшибки(Ош);
	КонецЕсли;
...
&НаСервере
Функция ПечатьНаСервереКазахстан(ТаблДокум)  
	Обработка = РеквизитФормыВЗначение("Объект"); 
	ТаблДокум.Очистить();
	Для Каждого стр из ТЧТовары Цикл
		ТипМакет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("МакетКазахстан");	
		
		ОбластьШапка = ТипМакет.ПолучитьОбласть("Шапка"); 
		ОбластьШапка.Параметры.Наименование	 = стр.Наименование; 
		мРезультат = СтрРазделить(стр.Номенклатура.НаименованиеПолное, " "); 
		
		ТаблДокум.Вывести(ОбластьШапка); 
		
		ОбластьСтрока = ТипМакет.ПолучитьОбласть("Строка"); 
		ОбластьСтрока.Параметры.Поставщик ="Поставщик: ООО 'СмартБар Групп', 117218, г.Москва, ул.Кржижановского, д 29,кор 5,пом.8Д/2/4 Срок годности и условия хранения: не более "+ Строка(стр.Номенклатура.СрокГодности)+" месяцев при температуре не выше +25°С и относительной владности воздуха не более 75%"+Символы.ПС + "Произведен по заказу ООО 'Спортмастер'";
		ДопСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("ИдентификаторДляФормул","ОписаниеКазахстан");
		ПараметрОписание = УправлениеСвойствами.ЗначениеСвойства(стр.Номенклатура, ДопСвойство);
		
		ОбластьСтрока.Параметры.Описание = ПараметрОписание;
		//       
		...
		ОбластьПодвал.Параметры.Масса = "Масса нетто: " + Строка(стр.Нетто) + " г. (" +Строка(стр.ВариантУпаковки.Числитель) + " шт по "+ Строка(стр.весшт) + " г. )";	
		ОбластьПодвал.Параметры.Масса1 = "Тара салмағы: " + Строка(стр.Нетто) + " г. (" +Строка(стр.ВариантУпаковки.Числитель) + " дана "+ Строка(стр.весшт) + " г. )";	
		ОбластьПодвал.Параметры.Дата = Формат(ТекДата, "ДЛФ=Д");
		ОбластьПодвал.Параметры.КодНом  = мРезультат[мРезультат.Количество()-1];
		ОбластьПодвал.Параметры.НД = Строка(стр.Номенклатура.ТипНД) +" "+ Строка(стр.Номенклатура.ЗначениеНД);
