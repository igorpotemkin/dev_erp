...
&НаКлиенте
Процедура Расш1_СХК_РЦНачалоВыбораПосле(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка=Ложь;  
	
	ПараметрыФормы = Новый Структура("Контрагент", Объект.Контрагент);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбработатьЗакрытиеФормыВыборРЦ", ЭтаФорма);
	
	ОткрытьФорму("Документ.ЗаказКлиента.Форма.СХК_ФормаВыбораРЦ", ПараметрыФормы,ЭтаФорма,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьЗакрытиеФормыВыборРЦ(Значение, ДопПараметры) Экспорт  
  
	Если ЗначениеЗаполнено(Значение) Тогда                  
    	Объект.СХК_РЦ = Значение.РЦ;  
		СХК_РЦПриИзмененииНаСервере();
	КонецЕсли;
...
&НаКлиенте
Процедура Расш1_СХК_РЦПриИзмененииПосле(Элемент)
	СХК_РЦПриИзмененииНаСервере();
КонецПроцедуры  

&НаСервере 
Процедура СХК_РЦПриИзмененииНаСервере()

	Если Объект.СХК_РЦ.СХК_Рентабельность = 0 Тогда
		ЭтаФорма.Элементы.Рентабельность.Заголовок = "Заполните рентабельность";
	Иначе
		ЭтаФорма.Элементы.Рентабельность.Заголовок = "";
	КонецЕсли;
