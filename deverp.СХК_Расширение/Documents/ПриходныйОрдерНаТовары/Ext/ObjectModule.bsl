...
Процедура Расш1_ОбработкаУдаленияПроведения(Отказ)  
	...
	ДокументПоступления =  СХК_ТСДСклад.ПроверкаДокументаПоступления( ЭтотОбъект.Ссылка);
	Если Не РольДоступна("ПолныеПрава") Тогда	
		Если ДокументПоступления <> Ложь Тогда
			Сообщить("Документ принят на склад " + Строка(ДокументПоступления));
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
