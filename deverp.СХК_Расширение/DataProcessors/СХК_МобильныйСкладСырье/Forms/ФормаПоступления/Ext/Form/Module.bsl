...
Функция ПринятьНаСервере()  
	...
	Если  Записи.Следующий() Тогда
		...
		ДокОсн = Записи.ЭтапПроизводства.Ссылка;	
	Иначе
		Результат.ТекстОшибки = "Запись не найдена";	
		Возврат Результат;		
	КонецЕсли;
		...
		Если Строки.Количество() >=1 Тогда
			...
			Иначе
				Результат.ТекстОшибки = "Не заполнена серия";	
				Возврат Результат;		
			КонецЕсли;
		Иначе
		...
		Исключение
			...
			Результат.ТекстОшибки = "Документ заказ на перемещение не записан";	
			Возврат Результат;		
		...
		Если РасхОрдер.ОформленныеОрдера.Количество() > 0 Тогда
			...
			Исключение
				...
				Результат.ТекстОшибки = "Документ Расходный ордер не записан";	
				Возврат Результат;		
			КонецПопытки; 
		...
		Исключение  
			ДокПТ.Записать(РежимЗаписиДокумента.Запись);
				Результат.ТекстОшибки = "Документ Перемещение Товаров не записан";	
				Возврат Результат;		
		...
		РезультатЗапросаПО = Запрос.Выполнить();
		СпособОбхода = ОбходРезультатаЗапроса.Прямой;
		ТабЗнач = РезультатЗапросаПО.Выгрузить(СпособОбхода);	
		Если ТабЗнач.Количество() = 1 Тогда
		...
		Если СкладПолучатель = Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции") Тогда 
			Если ДокПР.Ссылка.Проведен И ДокПТ.Ссылка.Проведен И РОрдер.Ссылка.Проведен И Док.Ссылка.Проведен Тогда			
				Запрос = Новый Запрос;
				Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
				...
				НоваяСтрока.ДокументОснование = ДокОсн;
				Попытка
					ТекДок.Записать(РежимЗаписиДокумента.Проведение);
					Ош = Ложь;
				Исключение
					Результат.ТекстОшибки = "Поступление в буфер не выполнено";	
					Возврат Результат;		
				Конецпопытки;
			КонецЕсли;
		КонецЕсли;
		...
		Иначе
			...
			Результат.ТекстОшибки = "Произошла ошибка выполните приемку в ручную";	
			Возврат Результат;		
		КонецЕсли; 
	//Иначе
...
&НаКлиенте
Процедура ПослеЗакрытияПредупреждениеПоступления(Параметры) Экспорт  
...
&НаКлиенте
Процедура АктивироватьСканНоменклатураПО() 
...
Функция ПроверитьСтатусПОрдера(УИД) 
	...
	Иначе
		Если Док.Статус = Перечисления.СтатусыПриходныхОрдеров.ТребуетсяОбработка И Док.Проведен Тогда
...
Процедура ПринятьПоОрдер(Команда) 
	...
	РезультатСервера  = ПринятьНаСервереПоОрдеру();
...
Функция ПринятьНаСервереПоОрдеру()  
	...
	Если ЭтаФорма.КоличествоОрдер*Числитель > 20000 Тогда
		Результат.ТекстОшибки = "Количество превышает 20000 шт";	
		Возврат Результат;		
	...
	Иначе
		Результат.ТекстОшибки = "Проведите документ приходный ордер";	
		Возврат Результат;		
	КонецЕсли;
	...
	РеквизитыШапки = Новый Структура; 
	РеквизитыШапки.Вставить("ВариантПриемкиТоваров",Перечисления.ВариантыПриемкиТоваров.РазделенаПоЗаказамИНакладным);
	РеквизитыШапки.Вставить("Организация",Справочники.Организации.НайтиПоНаименованию("Собинский Хлебокомбинат АО"));
	...
	Запрос.УстановитьПараметр("ПометкаУдаления",Ложь);
	Запрос.УстановитьПараметр("Распоряжение",Док.Ссылка);
	РезультатПО = Запрос.Выполнить();
	СпособОбхода = ОбходРезультатаЗапроса.Прямой;
	ТабЗнач = РезультатПО.Выгрузить(СпособОбхода);	
	Если ТабЗнач.Количество() = 1 Тогда
		ДокПР = ТабЗнач[0].Ссылка.ПолучитьОбъект();
	Иначе
		ДокПР = Документы.ПриходныйОрдерНаТовары.СоздатьДокумент();
	...
	Исключение 
		...
		Ош = Истина;
	КонецПопытки;
	МасДок.Добавить(ДокПР.Ссылка);
	////////////////////////////////////////////////
	Если СкладПолучатель = Справочники.Склады.НайтиПоНаименованию("ГП Склад готовой продукции") И ош = Ложь Тогда 
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
		|	Расш1_ПоступлениеСклад.Ссылка КАК Ссылка
		|ИЗ
		...
		НоваяСтрока.Пользователь = ЭтаФорма.Сотрудник;
		НоваяСтрока.ДокументОснование = ЭтаФорма.ДокОрдер.Ссылка;
	...
	Если ош = Истина Тогда
		...
		ЭтаФорма.Элементы.Ошибка.Заголовок = "Произошла ошибка выполните приемку в ручную";
		Результат.ТекстОшибки = "Произошла ошибка выполните приемку в ручную";	
		ОтменитьЦепочкуДокументов(МасДок);		
		Возврат Результат;		
	КонецЕсли; 
...
&НаСервере
Процедура ОтменитьЦепочкуДокументов(Мас)
	Для сч=0 По Мас.Количество()-1 Цикл
		ДокМ = Мас[сч].ПолучитьОбъект();
		Если ТипЗнч(ДокМ) = Тип("ДокументОбъект.ЗаказНаПеремещение") Тогда
			ДокМ.Статус = Перечисления.СтатусыВнутреннихЗаказов.КВыполнению;
			ДокМ.Записать(РежимЗаписиДокумента.Запись);
			
		КонецЕсли;
		ДокМ.Проведен = Ложь;
		ДокМ.Записать(РежимЗаписиДокумента.Запись);
		ДокМ.ПометкаУдаления = Истина;
		ДокМ.Записать(РежимЗаписиДокумента.Запись);
	КонецЦикла;
...
//////////////////////////////     
		...
		Если ДокПРСсылка.Проведен Тогда
			...
			Для каждого тСтр из табТоваровП Цикл
				...
				Если ЗначениеЗаполнено(тСтр.Серия.СХК_Упаковка) Тогда
					...
					НоваяСтрока.Количество = тСтр.КоличествоУпаковок*тСтр.Серия.СХК_Упаковка.Числитель;
				Иначе
				...
				НоваяСтрока.Ячейка = Справочники.СХК_СкладЯчейки.НайтиПоНаименованию("Буфер");;
				НоваяСтрока.Пользователь = ЭтаФорма.Сотрудник;
				НоваяСтрока.ДокументОснование = ДОрдер.Ссылка;
			КонецЦикла;
	...
	КонецЦикла;
		Если Ош = Истина Тогда
			ДОрдер = ЭтаФорма.ДокРО.ПолучитьОбъект();
			ДОрдер.Статус = Перечисления.СтатусыРасходныхОрдеров.КОтгрузке;
			ДОрдер.Записать(РежимЗаписиДокумента.Проведение);   
			ОтменитьЦепочкуДокументов(МасДок);		
		КонецЕсли; 
