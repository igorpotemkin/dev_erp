...
Процедура СформироватьП2(Команда)
	...
	Функция ПечатьНаСервереПриложение2()  
		...
		ОбластьШапка.Параметры.Заголовок =  "Отчет о роялти по Приложению № 1/1 и Приложению №2/1 к Договору № ЛИЦСГ-01082025 от 01.08.2025 г., заключенному между ООО «Союзмультфильм» и АО «Собинский хлебокомбинат»";
		ОбластьШапка.Параметры.НомерОтчета =  Строка(ЭтаФорма.НомерОтчета);   
		...
		ОбластьШапка.Параметры.ПериодОтчетаНачало =  Формат(ЭтаФорма.Период.ДатаНачала,"ДЛФ=Д");
		ОбластьШапка.Параметры.ПериодОтчетаОкончание =  Формат(ЭтаФорма.Период.ДатаОкончания,"ДЛФ=Д");
